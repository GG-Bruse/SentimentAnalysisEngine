...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):  # 获取单条信息
        ...
        tokens = config_inf.tokenizer.tokenize(self.contents[idx])  # 分词
        encode_dict = config_inf.tokenizer.encode_plus(
            text=tokens,
            add_special_tokens=True,
            max_length=config_inf.max_sequence_length,
            truncation=True,
            padding='max_length' if config_inf.max_sequence_length is not None else 'longest',  # 传参则填充至指定长度, 未传则填充到批次最长
            return_token_type_ids=True,
            return_attention_mask=True
        )
        input_ids = torch.LongTensor(encode_dict['input_ids']).to(config_inf.device)
        attention_mask = torch.LongTensor(encode_dict['attention_mask']).to(config_inf.device)
        segment_ids = torch.LongTensor(encode_dict['token_type_ids']).to(config_inf.device)
        label = self.labels[idx]
        label_tensor = torch.tensor(int(label)).to(config_inf.device)
        return (input_ids, segment_ids, attention_mask), label_tensor
